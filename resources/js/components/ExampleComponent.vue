  ...
  <div class="dropdown">
    ...
    >
      <div
        ...
        v-on:click="fxMarcarComoLeido(notificacion)"
        v-for="notificacion in notificaciones"
      >
        <div
          class="notification-icon"
          v-if="notificacion.data['notificacion']['tipo']=='comunicado'"
        >
        ...
        <div
          class="notification-icon"
          v-else-if="notificacion.data['notificacion']['tipo']=='anuncio'"
        >
        ...
        <div class="notification-details flex-grow-1">
          ...
          <p
            ...
            style="max-height: 19px; max-width: 150px; overflow: hidden"
          >{{notificacion.data['notificacion']['mensaje']}}</p>
        </div>
      </div>
      ...
      <div
        class="dropdown-item"
        v-on:click="fxMarcarTodoComoLeido()"
        v-if="notificaciones.length!=0"
      >Marcar todo como leido</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    fxMarcarComoLeido: function(notificacion) {
      ...
      axios
        .post("/notificacionesdelusuario/marcarcomoleido", data)
        .then(response => {
    ...
    fxMarcarTodoComoLeido: function() {
      axios
        .post("/notificacionesdelusuario/marcartodocomoleido")
        .then(response => {
          location.reload();
        });
    }
